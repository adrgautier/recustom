...
import { Mapper, StylableComponent, StylableProps, CompositeComponent } from '../types';

export default factory((componentName: string, customPropertiesMapper: Mapper, stateClassesMappers: Array<Mapper>) => (Component: CompositeComponent<StylableProps> | StylableComponent) => (
    class Style extends React.PureComponent<StylableProps> {
        ...
        get className() {
            const { state } = this;
            const { className: descendingClassName, style, bindState, ...props } = this.props;
            ...
            if(typeof bindState === "undefined") {
                ...
                return classNames(
                    componentName,
                    {
                        ...stateClassesMappers.reduce((classes, mapper) =>
                        ({...classes,
        ...
        get style() {
            const { state } = this;
            const { className, style: descendingStyle, bindState, ...props } = this.props;
        ...
        render() {
            ...
            return (
                <Component {...this.props} className={className} style={style} bindState={bindState} />
            );
        }
    }
));